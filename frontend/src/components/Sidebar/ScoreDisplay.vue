...
export default {
    ...
    methods: {
        ...
        async doMint() {
            ...
            const mod = await contract.getModule('suidouble_score');

            const moveCallParams = [
                maintainer,
                [{ type: 'SUI', amount: 200000000 }], // fee as vector<Coin<SUI>>
                mod.arg('u64', score),
                mod.arg('address', forAddress),
                mod.arg('vector<u8>', metadata),
            ];

            const res = await mod.moveCall('mint', moveCallParams);
